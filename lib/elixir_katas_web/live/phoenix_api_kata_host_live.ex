defmodule ElixirKatasWeb.PhoenixApiKataHostLive do
  use ElixirKatasWeb, :live_view
  alias ElixirKatas.Katas.DynamicCompiler
  import ElixirKatasWeb.KataComponents

  def mount(params, session, socket) do
    slug = Map.get(params, "slug", "00")
    kata_id =
    ...
    source_pattern = "lib/elixir_katas_web/live/phoenix_api_kata_#{kata_id}_*_live.ex"
    base_file =
    ...
    if base_file == nil do
      {:ok, socket |> put_flash(:error, "Phoenix API Kata #{kata_id} not found.") |> push_navigate(to: ~p"/phoenix-api-katas")}
    else
      ...
      title =
        base_file
        |> Path.basename(".ex")
        |> String.replace("phoenix_api_kata_", "Kata ")
      ...
      kata_name = "PhoenixApiKata#{kata_id}"
      ...
      # 4. Use phoenix_source/0 if the module exports it
      ...
      # 5. Load Notes
      notes_pattern = "notes/phoenix_api_kata_#{kata_id}_*_notes.md"
      notes_path =
