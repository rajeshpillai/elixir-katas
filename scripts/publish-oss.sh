...
OSS_REMOTE="${OSS_REMOTE:-https://github.com/algorisys-oss/elixir-katas.git}"
OSS_BRANCH="${OSS_BRANCH:-main}"                  # branch to push to on OSS remote
OSS_MESSAGE="${OSS_MESSAGE:-}"                    # custom commit message (optional)
OSSIGNORE_FILE="$SCRIPT_DIR/.ossignore"           # list of paths to exclude
DRY_RUN=true
...
if [[ "$DRY_RUN" == false && -z "$OSS_REMOTE" ]]; then
    error "OSS_REMOTE is not set. Export it or pass it inline:\n  OSS_REMOTE=https://github.com/algorisys-oss/elixir-katas.git $0 --push"
fi
